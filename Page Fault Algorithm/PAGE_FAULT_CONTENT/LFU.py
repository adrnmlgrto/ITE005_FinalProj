def run():
    ...
    print("LFU Enter the reference string: ", end="")
    ...
    for i in ref_str:
        ...
        if i not in frm_list:
            ...
            # CONDITION IF FRAME LIST IS FULL
                ...
                # REPLACEMENT OF THE PAGE THAT IS LEAST RECENTLY USED and Sorting the First Come First Serve Basis
                for j in range(len(frm_list)):
                    if frm_list[j] == least_recently_used:
                        frm_list[j] = i
                        tally[i] += 1
